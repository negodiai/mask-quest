...
// ========== УПРАВЛЕНИЕ МАСКАМИ В МАРШРУТАХ ==========

// Получить все маски в маршруте
router.get('/routes/:routeId/masks', checkAdmin, async (req, res) => {
    try {
...
router.post('/routes/:routeId/masks/:maskId', checkAdmin, async (req, res) => {
    ...
    try {
        ...
        const existing = await db.query(
            'SELECT * FROM route_masks WHERE "routeId" = $1 AND "maskId" = $2',
            [req.params.routeId, req.params.maskId]
        );
        
        if (existing.rows.length > 0) {
            return res.status(400).json({ error: 'Маска уже добавлена в этот маршрут' });
        }
        
        await db.query(`
            INSERT INTO route_masks ("routeId", "maskId", "order")
            VALUES ($1, $2, $3)
        `, [req.params.routeId, req.params.maskId, order || 999]);
...
// Обновить порядок маски в маршруте
router.put('/routes/:routeId/masks/:maskId', checkAdmin, async (req, res) => {
    const { order } = req.body;
    try {
        await db.query(`
            UPDATE route_masks 
            SET "order" = $1 
            WHERE "routeId" = $2 AND "maskId" = $3
        `, [order, req.params.routeId, req.params.maskId]);
        res.json({ success: true, message: 'Порядок маски обновлён' });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
...
router.delete('/routes/:routeId/masks/:maskId', checkAdmin, async (req, res) => {
    try {
        await db.query(
            'DELETE FROM route_masks WHERE "routeId" = $1 AND "maskId" = $2',
            [req.params.routeId, req.params.maskId]
        );
        res.json({ success: true, message: 'Маска удалена из маршрута' });
    } catch (err) {
